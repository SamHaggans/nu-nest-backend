...
def post_sublet_listing():
    ...
    cursor.execute('INSERT INTO Sublet_Listing(availability, roommate_count, bedroom_count, bathroom_count, \
                   ...
                   VALUES ({0}'.format(new_availability)+', {0}'.format(new_roommate_count)+',\
                     {0}'.format(new_bedroom_count)+', {0}'.format(new_bathroom_count)+', \
                     "{0}"'.format(new_start_date.split("T")[0])+', "{0}"'.format(new_end_date.split("T")[0])+', \
...
def put_sublet_listing(id):
    new_roommate_count = request.json['roommate_count']
    new_bathroom_count = request.json['bathroom_count']
    new_start_date = request.json['start_date'].split("T")[0]
    new_end_date = request.json['end_date'].split("T")[0]
    new_furnished_status = request.json['furnished_status']
    new_description = request.json['description']
    new_rent = request.json['rent']
    new_zipcode = request.json['zipcode']
    ...
    cursor.execute('UPDATE Sublet_Listing \
                    SET roommate_count = {0}'.format(new_roommate_count)+',\
...
# Get all reports on a specific user
@subletters.route('/user/<id>/report', methods=['GET'])
def get_user_reports(id):
    cursor = db.get_db().cursor()
    cursor.execute(f'select * from User_Report where reported_user = {id}')
...
# Get all ratings on a specific user
@subletters.route('/user/<id>/rating', methods=['GET'])
def get_user_ratings(id):
    cursor = db.get_db().cursor()
    cursor.execute(f'select * from Rating where rated_user = {id}')
    row_headers = [x[0] for x in cursor.description]
    json_data = []
    theData = cursor.fetchall()
    for row in theData:
